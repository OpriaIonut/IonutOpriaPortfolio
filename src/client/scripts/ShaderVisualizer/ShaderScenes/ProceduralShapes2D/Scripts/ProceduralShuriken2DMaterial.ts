...
//Settings which are used to render the shuriken
export declare type ProceduralShuriken2DUniforms = {
    //Shape settings
    u_ShurikenRadius: { value: number },    //How far from the center (0.5, 0.5) the shuriken ends will stretch
    u_NumPoints: { value: number },         //How many points the shuriken has
    u_MidPointDist: { value: number },      //How close the outer circles should be to the center (outer circles are the circles at the intersection of edges)
    u_MidPointRadius: { value: number },    //Radius of each mid point
    u_CenterRadius: { value: number },      //Radius of the center point

    //Color settings
    u_SharpSize: { value: number },         //Adds an edge on the outer side of the suriken
    u_SharpColor: { value: Color },
    u_BodyColor: { value: Color },          //Main color of the shuriken (typically should be a light color)
    u_ShadowColor: { value: Color },        //Additional color to add variation to the shuriken

    //Background settings
    u_BorderSize: { value: number },        //Adds a border on the outer edges of the plane
    u_BorderColor: { value: Color },        //Color to use for this border
    u_BackgroundColor: { value: Color },    //Color of the background (if opacity is not 0)
    u_BackgroundOpacity: { value: number }
}

export class ProceduralShuriken2DMaterial
{
    public static createMaterial(uniforms: ProceduralShuriken2DUniforms)
    {
        return new ShaderMaterial({
            transparent: true,
            vertexShader: proceduralShurikenVert,
            fragmentShader: proceduralShurikenFrag,
            uniforms: uniforms,
            side: DoubleSide
        });
    }
