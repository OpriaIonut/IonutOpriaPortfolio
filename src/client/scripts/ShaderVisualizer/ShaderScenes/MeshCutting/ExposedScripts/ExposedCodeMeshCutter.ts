...
export class MeshCutter
{
    ...
    {
        let vec1Pos = this.vec3Pool.reserve().copy(triangle.vert2.pos).sub(triangle.vert1.pos);
        let vec2Pos = this.vec3Pool.reserve().copy(triangle.vert3.pos).sub(triangle.vert1.pos);
        
        vec1Pos.cross(vec2Pos).normalize();
        this.vec3Pool.release(vec2Pos);
        return vec1Pos;
    }
